...
- **Antigravity CLI (agy)** = coding pesado, sobre el **free tier de OAuth** (login con tu cuenta Google en el host, compartido con el contenedor). Naru lo invoca con la key removida del entorno:
  ```bash
  cd /repo/workspace && env -u GEMINI_API_KEY -u GOOGLE_API_KEY -u GOOGLE_GENAI_USE_VERTEXAI agy --model "Gemini 3.1 Pro (High)" -p "..." --dangerously-skip-permissions
  ```
  El `env -u` es lo que evita que el CLI use (y gaste) la key de pago — fue el bug original.

#### 🔑 Login OAuth del CLI
El nuevo Antigravity CLI comparte el login activo del host montando la carpeta de credenciales en `docker-compose.yml`:
```yaml
- ~/.gemini/antigravity-cli:/root/.gemini/antigravity-cli
```
Si se necesita re-autenticar, se corre `agy` en el host directamente para iniciar sesión, lo cual actualiza automáticamente las credenciales usadas por el contenedor. No requiere interacción dentro de Docker.

> ⚠️ **Trust del workspace (obligatorio en headless):** el CLI no corre herramientas en un folder "no confiable" y falla con *"not running in a trusted directory"*. Por eso el `docker-compose.yml` setea `GEMINI_CLI_TRUST_WORKSPACE=true`. **No borres esa env var** o las delegaciones de Naru dejan de funcionar.
...
3. **Modelo del cerebro:** `auto` = `anthropic/claude-haiku-4-5` (Claude-Haiku orquestador, vía `ANTHROPIC_API_KEY`). El viejo cerebro flash (`gemini-2.5-flash` free tier) se jubiló por topar 429. El obrero sigue siendo el Antigravity CLI (OAuth free tier). Si OpenClaw rechaza el slug, corre `openclaw models list --provider anthropic` para el id exacto.
4. **Backlog:** Editar `workspace/BACKLOG.md` para darle trabajo a Naru. Toma la primera tarea `[ ]`.
5. **Login OAuth del CLI:** si Naru reporta fallo de auth del CLI, rehacer el login ejecutando `agy` en el host (ver sección 🔑).

---
*Nota: Se migró de gemini-cli (obsoleto) a Antigravity CLI (agy) y se configuró la compartición de credenciales de OAuth del host.*
